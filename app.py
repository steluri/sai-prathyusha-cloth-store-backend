...
PRODUCTS = [
    (1, "Linen Ease Shirt", "Women", 2499, 2999, "https://images.unsplash.com/photo-1596755094514-f87e34085b2c?auto=format&fit=crop&w=900&q=85", "Bestseller", "Oat", "A breezy linen-blend shirt with an easy, oversized silhouette."),
    (2, "Textured Knit Polo", "Men", 2899, None, "https://images.unsplash.com/photo-1617137968427-85924c800a22?auto=format&fit=crop&w=900&q=85", "New", "Sand", "A softly structured knit polo made for dressed-down days."),
    (3, "Sculpted Midi Dress", "Women", 3999, 4799, "https://images.unsplash.com/photo-1595777457583-95e059d581b8?auto=format&fit=crop&w=900&q=85", "Limited", "Terracotta", "Fluid lines and thoughtful tailoring in a day-to-evening dress."),
    (4, "Relaxed Pleat Trousers", "Women", 3199, None, "https://images.unsplash.com/photo-1594633312681-425c7b97ccd1?auto=format&fit=crop&w=900&q=85", None, "Stone", "High-rise trousers with soft front pleats and an elegant drape."),
    (5, "Everyday Overshirt", "Men", 3499, 3999, "https://images.unsplash.com/photo-1603252109303-2751441dd157?auto=format&fit=crop&w=900&q=85", "Bestseller", "Olive", "A versatile mid-weight layer with clean utility details."),
    (6, "Soft Rib Co-ord", "Women", 3599, None, "https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?auto=format&fit=crop&w=900&q=85", "New", "Cocoa", "An effortlessly polished ribbed set designed for all-day comfort."),
    (7, "Classic Camp Collar", "Men", 2299, 2699, "https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?auto=format&fit=crop&w=900&q=85", None, "Ivory", "A refined warm-weather shirt cut from breathable cotton."),
    (8, "Drape Studio Blazer", "Women", 4499, None, "https://images.unsplash.com/photo-1591369822096-ffd140ec948f?auto=format&fit=crop&w=900&q=85", "Limited", "Camel", "Relaxed tailoring with a soft shoulder and modern proportions."),
]
    ...
    with db() as con:
        con.executescript("""
            CREATE TABLE IF NOT EXISTS products (
                ...
                badge TEXT, color TEXT NOT NULL, description TEXT NOT NULL
            );
            ...
            CREATE TABLE IF NOT EXISTS orders (
                ...
                email TEXT NOT NULL, total INTEGER NOT NULL, items TEXT NOT NULL,
                created_at TEXT NOT NULL
            );
        """)
        existing_columns = {row["name"] for row in con.execute("PRAGMA table_info(products)")}
        for slot in IMAGE_SLOTS:
            column = f"image_{slot}"
            if column not in existing_columns:
                con.execute(f"ALTER TABLE products ADD COLUMN {column} TEXT")
        count = con.execute("SELECT COUNT(*) FROM products").fetchone()[0]
        if count == 0:
            con.executemany(
                """INSERT INTO products
                   (id, name, category, price, old_price, image, badge, color, description)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                PRODUCTS,
            )
            con.execute("UPDATE products SET image_front = image WHERE image_front IS NULL")
...
def admin_create_product():
    ...
    with db() as con:
        ...
        row = con.execute("SELECT * FROM products WHERE id = ?", (cursor.lastrowid,)).fetchone()
...
def admin_delete_product(product_id):
    with db() as con:
        ...
        if not existing:
            return jsonify({"error": "Product not found"}), 404
        con.execute("DELETE FROM wishlist WHERE product_id = ?", (product_id,))
...
def products():
    ...
    query += " ORDER BY id"
    with db() as con:
        return jsonify([dict(row) for row in con.execute(query, params).fetchall()])


@app.get("/api/wishlist")
def get_wishlist():
    with db() as con:
        rows = con.execute("SELECT p.* FROM products p JOIN wishlist w ON p.id = w.product_id ORDER BY p.id").fetchall()
...
def add_wishlist(product_id):
    with db() as con:
        if not con.execute("SELECT 1 FROM products WHERE id = ?", (product_id,)).fetchone():
            return jsonify({"error": "Product not found"}), 404
        con.execute("INSERT OR IGNORE INTO wishlist(product_id) VALUES (?)", (product_id,))
...
def create_order():
    ...
    with db() as con:
        rows = con.execute(f"SELECT id, price FROM products WHERE id IN ({placeholders})", ids).fetchall()
        ...
        cursor = con.execute(
            "INSERT INTO orders(customer, email, total, items, created_at) VALUES (?, ?, ?, ?, ?)",
            (customer, email, total, json.dumps(items), datetime.now(timezone.utc).isoformat()),
        )
    return jsonify({"order_id": cursor.lastrowid, "total": total, "status": "confirmed"}), 201
